...
try {
  ...
  // --- INICIO FLUJO RACI ---
  $stmtCom = $pdo->prepare("SELECT c.idLTYComunicacion, c.idLTYreporte, c.idusuario, c.rol, c.activo, c.fecha_asignacion, u.nombre, u.mail FROM LTYComunicacion c JOIN usuario u ON c.idusuario = u.idusuario WHERE c.idLTYreporte = :idLTYreporte AND c.activo = 's'");
  $stmtCom->execute([':idLTYreporte' => $idLTYreporte]);
  $usuarios = $stmtCom->fetchAll(PDO::FETCH_ASSOC);
  if (!$usuarios) {
    logMessage("No se encontraron usuarios RACI para el reporte $idLTYreporte");
  }
  foreach ($usuarios as $usuario) {
    ...
    $subject_rol = $subject;
    $html_rol = $html;
    $subject_rol = "[$rol] " . $subject_rol;

    // Personalización de {notificacion} según el rol (reemplazo aquí, no antes)
    switch (strtoupper($rol)) {
      case 'RESPONSABLE':
        $notificacion_rol = "&#9888;&#65039; Ud. $nombre_destino es RESPONSABLE de realizar una acción sobre este documento. Acceda a la plataforma con el número de documento para continuar.";
        $color_notificacion = '#1976d2';
        break;
      case 'APROBADOR':
        $notificacion_rol = "&#9888;&#65039; Ud. $nombre_destino es APROBADOR y debe aprobar este documento en la plataforma. Acceda con el número de documento.";
        $color_notificacion = '#388e3c';
        break;
      case 'CONSULTADO':
        $notificacion_rol = "&#128172; Ud. $nombre_destino está siendo CONSULTADO y debe responder a este correo o dejar su comentario en la plataforma.";
        $color_notificacion = '#fbc02d';
        break;
      case 'INFORMADO':
        $notificacion_rol = "&#8505;&#65039; Ud. $nombre_destino está siendo INFORMADO. No se requiere ninguna acción de su parte, solo es para su conocimiento.";
        $color_notificacion = '#607d8b';
        break;
      default:
        $notificacion_rol = $titulo;
    }
    $html_rol = str_replace('{color_notificacion}', $color_notificacion, $html_rol);
    $html_rol = str_replace('{notificacion}', $notificacion_rol, $html_rol);

    // Solo reemplazar {saludo_personalizado} si existe en el HTML base
    if (strpos($html_rol, '{saludo_personalizado}') !== false) {
      $html_rol = str_replace('{saludo_personalizado}', "Hola $nombre_destino ($rol),", $html_rol);
    }
    // echo 'html-rol>>>> ' . $html_rol;
    // Log temporal para depuración del cuerpo del email
    // logMessage("DEBUG HTML para $email_destino: " . substr($html_rol,0,200));
    ...
    // logMessage("Correo encolado para $email_destino ($rol)"); // <--- Elimino este log rutinario
  }
  // --- FIN FLUJO RACI ---
