...
try {
  ...
  // $encabezados =json_decode($encabezadox, true);
   ...
   if ($datos === null && json_last_error() !== JSON_ERROR_NONE) {
    ...
    ob_start();
    $htmlFilePath = $_SERVER['DOCUMENT_ROOT'] . '/iControl-Vanilla/icontrol/Nodemailer/emailFactum/email.html';
    if (!file_exists($htmlFilePath)) {
       die('Error: email.html no encontrado en ' . $htmlFilePath);
    }

    // Ahora puedes incluir el archivo si es necesario
