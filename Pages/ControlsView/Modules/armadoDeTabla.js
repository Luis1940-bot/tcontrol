...
function leyendaSubtituloTitulo(
  ...
  const texto = nombreControl.charAt(0).toUpperCase() + nombreControl.slice(1);
  let contenidoFilaUnido = texto;
  for (let i = 1; i <= 5; i++) {
    contenidoFilaUnido += '';
  }
...
function detectarPhoto(valor, nombreControl, fila) {
  try {
    ...
    const texto =
      nombreControl.charAt(0).toUpperCase() + nombreControl.slice(1);
    let contenidoFilaUnido = texto;
    for (let i = 1; i <= 5; i++) {
      contenidoFilaUnido += '';
    }
