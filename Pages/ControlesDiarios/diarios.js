...
// eslint-disable-next-line import/extensions
import tablaVacia from './Modules/armadoDeTablas.js';
...
const encabezados = {
  width: [0.25, 0.25, 0.25, 1, 0.5, 0.75],
};
const objButtons = {};
let objTranslate = [];
...
function dondeEstaEn(objTrad) {
  let lugar = `${trO('Controles Diarios', objTrad) || 'Controles Diarios'}`;
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (persona) {
    ...
    requestAnimationFrame(async () => {
      ...
      tablaVacia(objTranslate, plant, encabezados);
    });
  }
