...
async function readJSON(json, retries = 4, delay = 500) {
  ...
  for (let i = 0; i < retries; i++) {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Error al cargar ${json}.json: ${response.statusText} (${response.status})`,
        );
      }
      ...
      if (text.trim().startsWith('<') || text.trim().startsWith('<!DOCTYPE')) {
        throw new Error(
          `El servidor devolvió HTML en lugar de JSON para ${json}.json. Posible error 404 o redirección.`,
        );
      }

      // Verificar si parece ser un mensaje de error simple
      if (
        text.trim().startsWith('V') &&
        text.length < 10 &&
        !text.includes('{')
      ) {
        throw new Error(
          `El servidor devolvió texto plano "${text.trim()}" en lugar de JSON para ${json}.json`,
        );
      }
      ...
      try {
        ...
        throw new Error(
          `Error de formato JSON en ${json}.json: ${parseError.message}. Contenido recibido: ${text.substring(0, 200)}...`,
        );
      }
    } catch (error) {
