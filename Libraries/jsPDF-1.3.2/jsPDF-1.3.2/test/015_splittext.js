(function () {
  ...
  const test1 = function (jsPDF) {
    ...
    // splitStr picks up current font, size
		 const largetext = pdf.setFontSize(largetext_size).splitTextToSize(loremipsum, 7.5);
    // but allows you to pass in alternative fontName, fontStyle, fontSize
    // as part of settings Object.
		 let smalltext = pdf.splitTextToSize(loremipsum, 7.5, { fontSize: smalltext_size });

    drawMargins(pdf);

    pdf.text(0.5, 0.5 + largetext_size / 72, largetext);

    let offset = largetext.length * largetext_size / 72 + 2 * smalltext_size / 72;
