$(document).ready(() => {
  const dump = false; // turn to true to dump contents of PDFs into textareas instead.
  ...
  const datestringregex = /\/CreationDate \(D:\d+\)/;
  const replacementdatestring = '/CreationDate (D:0)';
  const producerstringregex = /\/Producer\s+\(jsPDF\s+\d+\)/;
  const replacementproducerstring = '/Producer  (jsPDF 0)';
  const removeMinorDiffs = function (t) {
