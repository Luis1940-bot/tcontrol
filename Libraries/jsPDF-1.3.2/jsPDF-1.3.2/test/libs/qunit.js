...
(function (window) {
  ...
  const Test = function (name, testName, expected, async, callback) {
    this.name = name;
  ...
  var QUnit = {
    ...
    test(testName, expected, callback, async) {
      ...
      const test = new Test(name, testName, expected, async, callback);
  ...
  extend(QUnit, {
    ...
    init() {
      ...
      if (qunit) {
        qunit.innerHTML =				`<h1 id="qunit-header">${document.title}</h1>`
    ...
    innerEquiv = function () { // can take multiple arguments
      ...
      return (function (a, b) {
        ...
        // apply transition with (1..n) arguments
				...
				&& arguments.callee.apply(this, args.splice(
				  1,
				  args.length - 1,
				));
    };
  ...
  QUnit.jsDump = (function () {
    ...
    var jsDump = {
      ...
      parsers: {
        ...
        function(fn) {
          let ret = 'function';
          const name = 'name' in fn ? fn.name : (reName.exec(fn) || [])[1];// functions never have name in IE
          if (name) ret += ` ${name}`;
          ret += '(';
